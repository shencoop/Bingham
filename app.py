...
if channel_type == "矩形人工明渠":
    h_e = np.cbrt(total_volume / (channel_w * L * 2)) 
...
with col_plot1:
    ...
    # 強制塞入個別字型，防止全域快取干擾
    ax1.set_xlabel("水平距離 (m)", fontproperties=my_font)
    ax1.set_ylabel("高程 (m)", fontproperties=my_font)
    ax1.set_title("渠道縱向剖面示意圖", fontproperties=my_font)
...
with col_plot2:
    ...
    if channel_type == "矩形人工明渠":
        ...
        ax2.fill_between([0, channel_w], [0, 0], [h_e, h_e], color='saddlebrown', alpha=0.7)
    else:
        ...
        ax2.fill_between(x_fluid, y_fluid, h_e, color='saddlebrown', alpha=0.7)
        
    ax2.set_xlabel("斷面寬度 (m)", fontproperties=my_font)
    ax2.set_ylabel("渠道深度 (m)", fontproperties=my_font)
    ax2.set_title(f"橫斷面幾何 ({channel_type})", fontproperties=my_font)
...
2. **與 2D 程式接軌：** 當你在此工具中調整出危險的組合（例如：流速 > 5 m/s 且流動距離極遠），可將本工具顯示的 `tau_0`, `mu_p`, `rho` 與推估出的**總流量/體積**，作為你 2D 偏偏微分方程模型的 `get_upstream_inflow(t)` 輸入邊界條件，進行更精確的河道氾濫與堆積範圍模擬。
""")
